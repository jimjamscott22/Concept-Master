...
SELECT t.id, tg.id FROM terms t, tags tg WHERE
  ...
  (t.slug IN ('array','linked-list','stack','queue','binary-search-tree','hash-map','heap','graph','big-o-notation','binary-search','merge-sort','quick-sort','depth-first-search','breadth-first-search','dynamic-programming','recursion','object','inheritance','polymorphism','encapsulation','closure','higher-order-function','thread','deadlock','rest','singleton','observer-pattern','garbage-collection','tcp','udp','authentication','authorization','sql-injection','memory-leak','race-condition') AND tg.name = 'java') OR
