---
title: Running a Split FastAPI + Node/Vite App
subtitle: Shipping and launching local-first apps with a Python backend and a JS frontend
is_published: true
categories: [web]
tags: [tooling, fundamentals, web]
related_terms: [cors, api, websocket]
related_articles: []
---
...
The core decision is one of *shape*: keep the backend and frontend as two separate processes on two ports (and deal with CORS/proxy), or fuse them so FastAPI serves the built frontend on a single port.

![Diagram contrasting Option 1 (two processes — browser talking to a Vite preview server on :5173 and uvicorn on :8000, needing CORS/proxy) with Option 2 (one process — browser talking to a single uvicorn on :8000 that serves both the static dist/ build and the /api routes, no CORS).](/concepts/running-split-apps.svg)
